...
const props = defineProps({
  ...
  focusPips: { type: Number, default: 0 },
  ...
  return [
    {
      ...
      label: 'Steady',
      hint: props.focusPips > 0 ? `focus ×${props.focusPips} (+${props.focusPips * 2} dmg)` : 'build focus',
    ...
    },
    {
      ...
      hint: props.enrageCharges >= 3 ? '2× dmg' : props.enrageCharges === 2 ? '+4 dmg' : props.enrageCharges === 1 ? '+2 dmg' : '0/3',
      confused: ca.includes('attack_enraged'),
      disabled: combatLocked.value || ca.includes('attack_enraged') || props.enrageCharges < 1 || lockedActions.value.some(a => a.action === 'attack_enraged') || fleeQueued.value || braceQueued.value || smokeBombQueued.value,
    },
    {
      action: 'special',
      icon: '✦',
