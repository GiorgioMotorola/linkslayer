export function handleMiniBossLootDrop({
  ...
  log(
    `<i class="ra ra-gold-bar"></i> <span class="player-name">${playerName.value}</span> finds ${goldAmount} Gold Pieces from defeating the ${defeatedEnemyData.name}.`
  );
  ...
  if (scrapAmount > 0) {
    ...
    log(
      `<i class="ra ra-cog"></i> <span class="player-name">${playerName.value}</span> salvages ${scrapAmount} Scrap Metal from the remains.`
    );
  }
}
